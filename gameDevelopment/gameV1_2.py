...
class Card:
    ...
    def play(self, user, target):
        # 检查是否被稻草人标记为无效
        if getattr(target, "_next_card_invalid", False):
            # 清除标记
            target._next_card_invalid = False
            return f"【无效化】{self.name} 的效果被稻草人抵消！"
        ...
        # 检查是否被稻草人标记为效果翻倍
        if getattr(target, "_next_card_amplified", False):
            # 清除标记
            target._next_card_amplified = False
            # 尝试解析结果并翻倍（只对HP和SAN有效）
            try:
                import re
                # 查找HP和SAN相关的数字
                hp_pattern = r"(?:造成|恢复|减少) (\d+) HP"
                san_pattern = r"(?:恢复|减少) (\d+) SAN"
                
                modified_result = result
                
                # 处理HP相关数值
                hp_matches = re.findall(hp_pattern, result)
                for num in hp_matches:
                    original_num = int(num)
                    doubled_num = original_num * 2
                    modified_result = modified_result.replace(f"{num} HP", f"{doubled_num} HP")
                    # 同时实际修改HP值
                    target.hp += doubled_num - original_num
                
                # 处理SAN相关数值
                san_matches = re.findall(san_pattern, result)
                for num in san_matches:
                    original_num = int(num)
                    doubled_num = original_num * 2
                    modified_result = modified_result.replace(f"{num} SAN", f"{doubled_num} SAN")
                    # 同时实际修改SAN值
                    if "恢复" in result:
                        target.san += doubled_num - original_num
                    else:  # 减少
                        target.san -= doubled_num - original_num
                
                # 处理普通攻击等直接修改HP的情况
                if "普通攻击" in self.name and "-1 HP" in result:
                    # 翻倍普通攻击的伤害
                    target.hp -= 1  # 额外增加1点伤害
                    modified_result = result.replace("-1 HP", "-2 HP")
                    
                # 如果有修改，返回修改后的结果
                if modified_result != result:
                    return f"【效果翻倍】{modified_result}"
            except:
                pass
            # 如果解析失败，简单添加效果翻倍标记
            return f"【效果翻倍】{result}"
...
def strawman(user, target, roll):
    if roll == 1:
        ...
        # 标记敌人的下一张牌效果翻倍（只对HP和SAN有效）
        target._next_card_amplified = True
        return f"{user.name} 稻草人（{roll}）→ {target.name} 下一张牌的HP和SAN效果将翻倍"
...
def mentos_god(user, target, roll):
    ...
    # 检查是否超过SAN上限
    actual_amount = min(amount, 10 - user.san)
    if actual_amount > 0:
        user.san += actual_amount
    ...
    # 检查是否超过HP上限
    actual_amount = min(amount, 10 - user.hp)
    if actual_amount > 0:
        user.hp += actual_amount
...
deck_prototypes = [
    ...
    Card("稻草人谬误", "概率干扰敌人", dice_sides=2, outcomes={(1,1): strawman, (2,2): strawman}, rarity=50),
...
def effect_status(player):
    effects = []
    if getattr(player, "_next_card_invalid", False):
        effects.append("下一张牌无效")
    if getattr(player, "_next_card_amplified", False):
        effects.append("下一张牌放大")
...
def game_demo(deck_size=DEFAULT_DECK_SIZE):
    ...
    while True:
        ...
        # 添加稻草人标记的特别提示
        if getattr(enemy, "_next_card_invalid", False):
            print(f"⚠️ {current.name} 注意：{enemy.name} 的稻草人标记已生效，你的下一张牌将被无效化！")
        if getattr(enemy, "_next_card_amplified", False):
            print(f"⚠️ {current.name} 注意：{enemy.name} 的稻草人标记已生效，你的下一张牌效果将被放大！")
