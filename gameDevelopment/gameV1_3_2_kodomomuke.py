...
def blood_attack(user, target, roll):
    if 1 <= roll <= 4:
        ...
        return f"{user.name} 起床气 成功（{roll}）→ {target.name} -3 HP，自身 -1 SAN"
    else:
        user._san_modifier -= 2  # 使用中间变量
        return f"{user.name}  失败（{roll}）→ 自身 -2 SAN"
...
def slow_down(user, target):
    user.dice_speed *= 1.6
    return f"{user.name} 使用 慢速表 → 本人掷骰闪现变慢（{user.dice_speed:.3f}s）"

def speed_up(user, target):
    target.dice_speed = max(0.02, target.dice_speed * 0.5)  # 加速对手的骰子（向下限靠拢）
    return f"{user.name} 使用 加速表 → {target.name} 掷骰闪现变快（{target.dice_speed:.3f}s）"
...
def twilight_lizard(user, target, roll):
    if 14 <= roll <= 17:
        target._hp_modifier -= 1  # 使用中间变量
        return f"{user.name} 夕阳（{roll}）→ {target.name} -1 HP"
    elif 18 <= roll <= 19:
        target._hp_modifier -= 4  # 使用中间变量
        return f"{user.name} 夕阳（{roll}）→ {target.name} -4 HP"
    elif 20 <= roll <= 24:
        target._hp_modifier -= 2  # 使用中间变量
        return f"{user.name} 夕阳{roll}）→ {target.name} -2 HP"
    else:
        return f"{user.name} 夕阳（{roll}）→ 无效点数，没有效果"
...
def qiu_luo_effect(user, target):
    ...
    return f"{user.name} 使用 盲仔 → {target.name} 的下一次骰子将显示乱码"
...
def void_box_effect(user, target, roll):
    ...
    return f"{user.name} 使用 递归（{roll}）→ {target.name} 的下一次判定需要递归 {n} 次"

# ====== 牌库原型模板（每种卡只定义一次，下面会根据 rarity 生成具体副本） ======
deck_prototypes = [
    Card("普通攻击", "造成 1 HP", stable_effect=normal_attack, rarity=20),
    Card("起床气", "高伤害但有风险", dice_sides=6, outcomes={(1,6): blood_attack}, rarity=60,
         subranges=[(1, 4), (5, 6)]),

    Card("周末偷懒", "可能让敌人跳过回合", dice_sides=7, outcomes={(1,7): weekend}, rarity=70,
         subranges=[(1, 5), (6, 7)]),
    Card("慢速表", "使自己掷骰变慢", stable_effect=slow_down, rarity=40),
    Card("加速表", "使对手掷骰变快", stable_effect=speed_up, rarity=40),
    ...
    Card("夕阳", "14-16:扣1HP, 17-19:扣4HP, 20-24:扣2HP", dice_sides=24, outcomes={(14,16): twilight_lizard, (17,19): twilight_lizard, (20,24): twilight_lizard}, min_value=14, rarity=50,
         subranges=[(14, 17), (17, 19), (20, 24)]),
    Card("睡午觉", "当前回合-2SAN，四回合后+5SAN", stable_effect=chicken_machine, rarity=65),
    Card("眼镜蛙", "1-5看片+2SAN, 6-10飞踢-2HP", dice_sides=10, outcomes={(1,10): glasses_frog_effect}, rarity=50,
         subranges=[(1, 5), (6, 10)]),
    Card("盲仔", "使对方下一次骰子显示乱码", stable_effect=qiu_luo_effect, rarity=80),
    Card("递归", "使对方下次判定需要递归1-3次", dice_sides=5, outcomes={(1,5): void_box_effect}, rarity=80,
...
def show_game_intro():
    print("\n===== 游戏介绍 =====")
    print("欢迎来到这个充满策略与运气的卡牌对战游戏！在这个游戏中，你将扮演一位卡牌决斗者，通过巧妙地使用各种卡牌来击败对手。\n")
    
    print("=== 基础规则 ===")
    print("• 每位玩家初始拥有 10点生命值(HP) 和 10点理智值(SAN)")
    print("• 当你的HP或SAN降至0或以下时，你将输掉比赛")
    print("• 每回合你有一定的 行动力，行动力等于你当前HP值的一半（最少为2点）")
    print("• 每使用一张卡牌会消耗1点行动力")
    print("• 每回合结束时，你会抽取1张新卡，并需要将手牌数量调整至不超过你的行动力上限\n")
    
    print("=== 骰子系统 ===")
    print("• 许多卡牌需要掷骰子来决定效果")
    print("• 骰子会以闪现的形式显示，增加游戏的紧张感和趣味性")
    print("• 某些卡牌可以影响骰子的显示速度或结果\n")
    
    print("=== 特殊机制 ===")
    print("• 延迟效果：某些卡牌的效果会在未来几回合后触发")
    print("• 负行动力：某些效果会让你累积负行动力，导致你在接下来几个回合无法行动")
    print("• 跳过回合：某些效果会让对手跳过下个回合")
    print("• 递归判定：某些卡牌会让对手在判定时需要多次掷骰，增加不确定性\n")
    
    print("=== 卡牌类型 ===")
    print("【攻击型卡牌】")
    print("• 普通攻击：造成1点伤害，简单直接")
    print("• 起床气：高风险高回报，有几率造成3点伤害，但失败会损失理智值")
    print("• 夕阳：根据骰子结果造成不同伤害，最高可造成4点伤害\n")
    
    print("【控制型卡牌】")
    print("• 周末偷懒：有机会让对手跳过下个回合")
    print("• 慢速表：减慢自己骰子的闪现速度，增加思考时间")
    print("• 加速表：加快对手骰子的闪现速度，给对手施加压力")
    print("• 盲仔：让对手下一次骰子显示为乱码，增加不确定性")
    print("• 递归：让对手下次判定需要多次掷骰，增加失败概率\n")
    
    print("【恢复型卡牌】")
    print("• 曼妥思之神：通过两次判定分别恢复SAN和HP，是强大的恢复卡牌")
    print("• 睡午觉：立即损失2点SAN，但四回合后恢复5点SAN，需要长远规划\n")
    
    print("【特殊卡牌】")
    print("• 300龟：极低概率触发，但一旦成功可强制减少对手300点HP、SAN或行动力")
    print("• 眼镜蛙：根据骰子结果，要么恢复SAN，要么对敌人造成伤害")
    print("• 调试卡牌：仅在调试模式可用，可设置下一张牌的点数\n")
    
    print("=== 游戏策略提示 ===")
    print("1. 平衡进攻与防守：不要只专注于攻击对手，也要注意保护自己的HP和SAN")
    print("2. 合理利用行动力：规划好每回合的行动，优先使用关键卡牌")
    print("3. 手牌管理：注意手牌数量，避免在回合结束时被迫弃掉重要卡牌")
    print("4. 把握时机：某些卡牌在特定时机使用效果更佳，比如在对手行动力低时使用控制卡牌")
    print("5. 长期规划：考虑延迟效果的触发时机，提前布局\n")
